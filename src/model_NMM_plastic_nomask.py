...
class nmm_plastic:
    def __init__(self,):
        self.dt = 1./1024. #initial value for dt
        self.NbODEs = 16 # number of ODE equations to solve
        self.NbODEsPlast = 7 #number of plasticity ODE equations to solve
    ...
    def init_vector_simple(self):
        self.dydx = np.zeros((self.NbODEs,self.NbNMMs)) # matrix for the first derivative of RK4
        self.dydx1 = np.zeros((self.NbODEs,self.NbNMMs)) # matrix for the second derivative of RK4
        self.dydx2 = np.zeros((self.NbODEs,self.NbNMMs)) # matrix for the third derivative of RK4
        self.dydx3 = np.zeros((self.NbODEs,self.NbNMMs)) # matrix for the fourth derivative of RK4
    ...
    def init_vector(self):
        ...
        self.ExtI_I2_I2     = np.zeros((self.NbNMMs)) #vector that receive external I2 to I2 inputs
        ...
        self.CM_I2_I2 = np.zeros((self.NbNMMs, self.NbNMMs)) # connectivity matrix between P and I4
        self.DelayMat = np.zeros((self.NbNMMs, self.NbNMMs))# delay matrix
        self.Delay_in_index_Mat = np.zeros((self.NbNMMs, self.NbNMMs), dtype=np.int32)# delay used to compute a buffer delay matrix (not used here)
        self.EmptyMat = np.full((6), True)# vector to know if some connectivity matric are all zeros to save computation time (not apply dot product if the matrix is all zero)
        self.history_pulseP = np.zeros((self.NbNMMs, 1))# delay in number of sample
        self.history_pulseI2 = np.zeros((self.NbNMMs, 1))  # delay in number of sample
        ...
        self.pulseI2_delay_Mat = np.zeros((self.NbNMMs, self.NbNMMs))# buffer delay matrix
        ...
        self.p2 = np.ones((1, self.NbNMMs)) * 6 #THIS NEEDS TO BE MODIFIABLE FROM THE GUI, FOR NOW THERE IS NO REASON FOR SIGMOID TO HAVE ANOTHER STABLE POINT THAN v0 = 6 BUT MIND THIS IS HARDCODED HERE AND NEEDS CHANGING LATER
    ...
    #########
    def func_omega_p(self, v): # ⌂ (omega) gives how synaptic efficacy is controlled by calcium concentration (x)
        # gamma is the rate of potentiation/depression
        # alpha is potentiation/depression boundaries
        return self.omega_gamma_p/(1+np.exp(-self.omega_beta_p*(v - self.omega_alpha_p))) # NEED TRANSPOSES


    def func_omega_d(self, v): # ⌂ (omega) gives how synaptic efficacy is controlled by calcium concentration (x)
        # gamma is the rate of potentiation/depression
        # alpha is potentiation/depression boundaries
        return self.omega_gamma_d/(1+np.exp(-self.omega_beta_d*(v - self.omega_alpha_d))) #NEED TRANSPOSES

    def sigmNMDApost(self, v):
        return self.sigm_NMDApost_amplitude/(1 + np.exp(-self.sigm_NMDApost_slope*(v - self.sigm_NMDApost_threshold))) #ERROR?
    ...
    def derivT(self , ):
        ...
        self.EEGoutput = (self.CP1P * self.y[8,:]
                          ...
                          + self.y[10,:]
                          #+ self.y[12,:]
                          + np.sum(self.p[2, :, :] * self.p[3, :, :] + self.campad * (self.p[5, :, :].T * self.sigmNMDApost(self.EEGoutput)).T, axis=1)) #POST-SYNAPTIC POTENTIAL


        self.OutSigmoidEXC = self.sigmP(self.k_P * self.Stim * self.Pre_Post+ self.EEGoutput) #compute FIRING RATE, without stimulation affecting this firing rate. (stim now affects only sigmoid).
        ...
        self.OutSigmoidI1 = self.sigmI1(self.k_I1 * self.Stim + self.k_I1 * self.Stim * self.Pre_Post
                                        ...
                                        - self.CI1I1 * self.y[2,:]
                                        - self.CI2I1 * self.y[6,:])# compute sigmoid on the firing rate of I1 population and apply stimulation if applyed before sigmoide
        ...
        self.OutSigmoidI2 = self.sigmI2(  self.k_I2 * self.Stim * self.Pre_Post
                                          + self.CPI2*self.y[0,:])# compute sigmoid on the firing rate of I2 population and apply stimulation if applyed before sigmoide
        self.dydx[6,:] = self.y[7,:]
        self.dydx[7,:] =  self.PSP(self.k_I2 * self.Stim * (not self.Pre_Post)
                                   + self.OutSigmoidI2 ,self.y[6,:],self.y[7,:], self.B, self.b1, self.b2)# compute H transfert function on Firing rate I2 signal and apply stimulation if applyed after sigmoide
        ...
        # Equations for input noise Pyramidal  #16 = 10, 17 = 11
        ...
        self.dydx[15,:]  =   self.PSP(self.k_I2 * self.Stim * (not self.Pre_Post)
                                      + self.OutSigmoidI2, self.y[14,:],self.y[15,:], self.BB, self.bb1, self.bb2)# compute H transfert function on Firing rate Pp signal and apply stimulation if applyed after sigmoide


        #plasticity ODEs:
        self.dydP[0, :, :] = (self.p[5, :, :].T * self.campad * self.sigm_NMDApost_Ca_factor * self.sigmNMDApost(self.EEGoutput)).T - self.p[0, :, :] / self.tauCa

        # synaptic efficacy:
        self.dydP[1, :, :] = (-1 * self.p[1, :, :] * (1 - self.p[1, :, :]) * (0.5 - self.p[1, :, :]) + (1 - self.p[1, :, :]) * self.func_omega_p(self.p[0, :, :]) - self.p[1, :, :] * self.func_omega_d(self.p[0, :, :])) / self.tauP

        # AMPAr count:

        self.dydP[4, :, :] = self.PSP(self.OutSigmoidEXC * self.CM_P_P, self.p[3, :, :], self.p[4, :, :],
                                      self.A_ampa,
                                      self.aa_ampa,
                                      self.aa_ampa)

        # NMDA afference (synapse specific):
        self.dydP[5, :, :] = self.p[6, :, :]

        self.dydP[2, :, :] = (self.campad - self.p[2, :, :] + self.p[1, :, :] * (self.campap - self.campad)) / self.tau_campa

        # AMPA afference (synapse specific):
        self.dydP[3, :, :] = self.p[4, :, :]
        self.dydP[6, :, :] = self.PSP(self.OutSigmoidEXC * self.CM_P_P, self.p[5, :, :], self.p[6, :, :], #OutSigmoidEXC is now plastic: dydP_acute.
                                      self.A_nmda,
                                      self.aa_nmda,
                                      self.aa_nmda)
        ...
        self.PPSI2apical = self.CI2aP*self.y[14,:] # get PSP of I2 apical projection on P
        self.PPSExtI_P_P = self.y[12,:] + self.y[10,:]# get PSP of External Input on PYR
...
if __name__ == '__main__':
    ...
    lfp = C.Eul_Time_Maruyama(10)


    import matplotlib.pyplot as plt
    plt.figure()
    for i in range(C.NbNMMs):
